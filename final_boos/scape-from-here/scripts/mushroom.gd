...
@onready var raycast_L: RayCast2D = $CharacterBody2D/RayCast_L
@onready var ray_cast_R: RayCast2D = $CharacterBody2D/RayCast_R
...
func handle_hit_state() -> void:
	...
	if current_state == State.HIT:
		...
		if direction != real_direction_to_player and real_direction_to_player != 0:
			direction = real_direction_to_player 
...
func _on_hitbox_area_entered(area: Area2D) -> void:
	area.take_damage(attack, character_body_2d.global_position)
